...
table2latex(table, 'ASCA0.tex', '%.2f');
...
uc = unique(class);
for u = 1:length(up)
    class(find(ismember(pac_l,up(u)))) = uc((randn<=0)+1);
end

Ffalse = [string(class), string(timel), string(pac_l)];

%% ASCA Model M1 considered X = 1mT + A + B + AB + E, Statistic MSa

[table, struct] = parglm(X, Ffalse,'Preprocessing',2,'Model', [1 2], 'Fmtc', 0, 'Nested', [1 3], 'Ts', 0, ...
    'Random', [0 0 1]);
table.Source(1:3)={"Responder","Time","Patient"}

table2latex(table, 'ASCA1.tex', '%.2f');


%% ASCA Model M2 considered X = 1mT + A + B + C + AB + E,  Statistic F = MSa/MSe 

[table, struct] = parglm(X, Ffalse,'Preprocessing',2,'Model', [1 2], 'Fmtc', 0, 'Nested', [1 3], ...
    'Random', [0 0 1]);
table.Source(1:3)={"Responder","Time","Patient"}
...
table2latex(table, 'ASCA4.tex', '%.2f');
...
table2latex(table, 'ASCA5.tex', '%.2f');
