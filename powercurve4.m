...
% Modification from powercurve at MEDA Toolbox (see lines 614 to 642) where
% the model is based on LMM.
%
...
addParameter(p,'NPCs',min(10,M)); %!!!HERE
parse(p,varargin{:});
...
% Here!!! LLM
eD = zeros(length(theta),3*length(powercurveo.coeffs),nRep);
% End Here
...
for i2=1:nRep
    ...
    if type == 1 % Relative PCs
        ...
        for a = 1:length(theta)
            ...
            % Parallel GLM
            % Here!!! LLM
            options.iterations = nPerm;
            options.baseline = 'ucLDA'; % unconstrained (for main effect in A)
            ...
            if prep == 2
                [Xm,m,sd] = preprocess2D(Xm,'Preprocessing',2);
                Xm = Xm + ones(size(Xm,1),1)*m;
            end

            data2 = table();
            data2.ID = F(:,3);
            data2.ID(size(F,1)/2+1:end)=data2.ID(size(F,1)/2+1:end)+max(F(:,3));
            data2.timepoint = F(:,2);
            data2.treatment = F(:,1);
            Y = array2table(Xm);
            data = [data2, Y];
            ...
            [MB, MA, MAB, MC] = RM_LiMM_PCA_simV(data, options);

            %[T, parglmo] = parglm(Xm, F, 'Model', model, 'Preprocessing', prep, 'Permutations', nPerm, 'Ts', ts, 'Ordinal', ordinal, 'Random', random, 'Fmtc', fmtc, 'Coding', coding, 'Nested', nested2);
...
ylabel('Power (F random)','FontSize', 16);
