...
% Modification from powercurve at MEDA Toolbox (see lines 585 to 590), 
% data is generated with a nested individual, but the models do not 
...
for i2=1:nRep
    ...
    if type == 1 % Relative PCs
        ...
        for a = 1:length(theta)
            ...
            sel = 1:2;
            nested2 = [];
            [T, parglmo] = parglm(Xm, F(:,sel), 'Warning', false, 'Parallel', false, 'Model', model, 'Preprocessing', prep, 'Permutations', nPerm, 'Ts', ts, 'Ordinal', ordinal(sel), 'Random', random(sel), 'Fmtc', fmtc, 'Coding', coding(sel), 'Nested', nested2);
            parglmo.p = [parglmo.p(1:2) 1 parglmo.p(3)];
            % End Here
